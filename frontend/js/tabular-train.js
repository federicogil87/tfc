        ...
        trainingForm.addEventListener('submit', async function(e) {
            ...
            for (const [key, value] of formData.entries()) {
                if (key.startsWith('model_params.')) {
                    ...
                    if (['n_estimators', 'max_depth', 'min_samples_split', 'n_neighbors'].includes(paramName)) {
            ...
            if (trainingData.problem_type === 'classification') {
                trainingData.num_classes = 2; // Por defecto binaria
            }
...
function validateTrainingData(data) {
    ...
    if (data.num_samples < 50) {
        showAlert('El número de muestras debe ser al menos 50', 'danger', alertsContainer);
        return false;
    }
    
    // Validar número de características
    if (data.num_features < 2) {
        showAlert('El número de características debe ser al menos 2', 'danger', alertsContainer);
        return false;
    }
    
    // Validar proporción de test
    if (data.test_size < 0.1 || data.test_size > 0.5) {
        showAlert('La proporción de test debe estar entre 0.1 y 0.5', 'danger', alertsContainer);
        return false;
    }
            ...
            <div class="results-summary">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td>${results.evaluation?.problem_type === 'classification' ? 'Clasificación' : 'Regresión'}</td>
                    </tr>
                </table>
            </div>
        `;
            ...
            if (results.evaluation.accuracy !== undefined) {
                ...
                html += `
                    ...
                    <table>
                        ...
                        <tr>
                            <th>Precision:</th>
                            <td>${(results.evaluation.precision * 100).toFixed(2)}%</td>
                        </tr>
                        ` : ''}
                        ${results.evaluation.recall ? `
                        <tr>
                            <th>Recall:</th>
                            <td>${(results.evaluation.recall * 100).toFixed(2)}%</td>
                        </tr>
                        ` : ''}
                        ${results.evaluation.f1 ? `
                        <tr>
                            <th>F1 Score:</th>
                            <td>${(results.evaluation.f1 * 100).toFixed(2)}%</td>
                        </tr>
                        ` : ''}
                    </table>
                `;
    ...
    let html = `
        <div class="confusion-matrix mt-4">
            ...
            <table>
                <tr>
                    <th></th>
                    ${Array.from({ length: numClasses }, (_, i) => `<th>Pred ${i}</th>`).join('')}
                </tr>
    `;
...
function createFeatureImportanceSection(featureImportance) {
    ...
    if (featureImportance.importances) {
        ...
        importances = Object.entries(featureImportance).map(([feature, importance]) => ({
            feature,
            importance: typeof importance === 'number' ? importance : Math.abs(importance)
        }));
    }
        ...
        html += `
            <div class="feature-bar">
                ...
                <div class="feature-value-bar">
                    <div class="feature-value-fill" style="width: ${normalizedValue * 100}%"></div>
                    <span class="feature-value">${item.importance.toFixed(4)}</span>
                </div>
            </div>
        `;
    });
